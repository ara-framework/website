...
We know the Frontend technologies evolve pretty fast. Nowadays, [Svelte](https://svelte.dev) has become very popular, and more developers have started using it. However, currently there's no any framework like Nuxt.js or Next.js to create universal applications using Svelte. But what if we could use it in Nuxt.js?.

This is a perfect scenario where we can take advantage of the Tech Agnostic principle of Microfrontends. So, Why we need to wait until something so cool like Nuxt.js for Svelte come up? We don't. We can re-use what already exists.
